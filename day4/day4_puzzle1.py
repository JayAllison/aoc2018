...
for guard in guards:
    sleepy_time = sum(guards[guard])
    if sleepy_time > max_sleepy_time:
        max_sleepy_time = sleepy_time
        sleepiest_guard = guard

sleepiest_minute = 0

for i in range(60):
    if guards[sleepiest_guard][i] > guards[sleepiest_guard][sleepiest_minute]:
        sleepiest_minute = i
    # print(f'{i}: {guards[sleepiest_guard][i]} {sleepiest_minute}')
